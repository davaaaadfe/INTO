...
Review follow-up: PostgreSQL now owns a checksum derived from its canonical DDL and schema version; both drivers normalize and constrain lowercase email storage; and both repositories expose the shared idempotent legacy-user/event migration operation. The PostgreSQL contract covers future-version and stale-checksum rejection. The immutable v1 schema/checksums remain supported; v2 upgrades canonicalize email only after preflight collision checks and fail closed without a v2/data write on collisions.

TDD evidence: the initial auth test failed because the repository module did not exist; the configured-repository and raw-inventory edge tests likewise failed before their corresponding minimal implementations. Review regression tests failed before their corresponding fixes. Focused suite: 18/18 passing. Full suite: 327/327 passing. `pnpm run typecheck` and `pnpm run lint` pass.
