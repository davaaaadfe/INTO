...
export function IntoUsersPanel({ bootstrapOnly = false }: { bootstrapOnly?: boolean }) {
  ...
  async function invite(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!bootstrapOnly) await loadUsers();
    } catch (error) {
  ...
  return (
    <section className="rounded-lg border border-stone-300 bg-white p-4" aria-labelledby="users-title">
      ...
      <p className="mt-1 text-sm text-stone-500">
        {bootstrapOnly ? "Invite the first verified people." : "Invite verified people and manage account status."}
      </p>
      ...
      {!bootstrapOnly ? <ul className="mt-4 divide-y divide-stone-200">
