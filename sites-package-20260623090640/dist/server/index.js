globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/into-workbench-Cxn806vT.js","assets/layout-segment-context-AJt1nybn.js"];
...
function getDraftSecret() {
	return "47da85ea-e15a-4b1e-9157-b7b1b6215c8d";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "371215be-2cf0-4886-aeac-e0f17ef13a99";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "371215be-2cf0-4886-aeac-e0f17ef13a99") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "371215be-2cf0-4886-aeac-e0f17ef13a99",
		rootBoundaryId
	});
}
/**
...
async function exactCryptoKey() {
	const secret = process.env.EXACT_TOKEN_ENCRYPTION_KEY;
	if (!secret) throw new Error("EXACT_TOKEN_ENCRYPTION_KEY is required for real Exact Online token storage.");
...
async function signExactState(payload) {
	const secret = process.env.EXACT_OAUTH_STATE_SECRET || process.env.EXACT_TOKEN_ENCRYPTION_KEY;
	if (!secret) throw new Error("EXACT_OAUTH_STATE_SECRET or EXACT_TOKEN_ENCRYPTION_KEY is required for real Exact Online OAuth.");
...
async function syncExactDataNow(userId = getCurrentUser().id) {
	const store = getStore();
	const connection = await refreshExactTokenIfNeeded(getExactConnection(userId));
...
async function GET$10(_request, context) {
	try {
...
async function POST$9(_request, context) {
	...
	try {
		const connection = await refreshExactTokenIfNeeded(getExactConnection());
		if (connection) setExactConnection(connection);
...
var route_exports$14 = /* @__PURE__ */ __exportAll({ POST: () => POST$8 });
...
var route_exports$9 = /* @__PURE__ */ __exportAll({ GET: () => GET$6 });
...
var route_exports$8 = /* @__PURE__ */ __exportAll({ POST: () => POST$5 });
...
async function POST$3() {
	...
	try {
		connection = await refreshExactTokenIfNeeded(getExactConnection());
		if (connection) setExactConnection(connection);
		if (isCachedExactMasterDataStale()) masterData = await syncExactDataNow();
	} catch (error) {
		const message = error instanceof Error ? error.message : "Exact master-data sync failed.";
...
function createOutlookAuthorizationUrl(state) {
	...
	const redirectUri = process.env.MICROSOFT_REDIRECT_URI || "http://localhost:3000/api/outlook/connect";
