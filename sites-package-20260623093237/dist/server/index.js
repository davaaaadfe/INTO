globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/into-workbench-DPFii_yd.js","assets/layout-segment-context-CxtYWCMp.js"];
...
function getDraftSecret() {
	return "f875365e-713a-4a75-a130-1b1ff92a12af";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "605546c4-e936-4551-a0fd-3f29d442c6f8";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "605546c4-e936-4551-a0fd-3f29d442c6f8") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "605546c4-e936-4551-a0fd-3f29d442c6f8",
		rootBoundaryId
	});
}
/**
...
//#endregion
//#region lib/domain/invoice.ts
var emptyExtractedInvoiceData = () => ({
	supplierName: "",
	supplierVatNumber: "",
...
//#endregion
//#region lib/utils/id.ts
function createId(prefix) {
	return `${prefix}_${typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`}`;
}
//#endregion
//#region lib/services/invoice-validation.ts
var currencyPattern = /^[A-Z]{3}$/;
var datePattern = /^\d{4}-\d{2}-\d{2}$/;
function error(field, message, severity = "error") {
	return {
		id: createId("val"),
		field,
		message,
...
function validateRequiredString(data, field, label, errors) {
	const value = data[field];
	if (typeof value !== "string" || !value.trim()) errors.push(error(field, `${label} is required.`));
}
function validateAmount(data, field, label, errors) {
	const amount = amountToMinorUnits(data[field]);
	if (amount === null) {
		errors.push(error(field, `${label} must be a valid amount with up to two decimals.`));
		return null;
	}
	if (amount < 0) errors.push(error(field, `${label} cannot be negative.`));
	return amount;
}
function validateInvoiceData(invoiceId, data, duplicateCandidates) {
	const errors = [];
	...
	if (data.invoiceDate && !isValidIsoDate(data.invoiceDate)) errors.push(error("invoiceDate", "Invoice date must be a valid YYYY-MM-DD date."));
	if (data.dueDate && !isValidIsoDate(data.dueDate)) errors.push(error("dueDate", "Due date must be a valid YYYY-MM-DD date."));
	if (data.currency && !currencyPattern.test(data.currency.trim())) errors.push(error("currency", "Currency must be a three-letter ISO code."));
	...
	if (netAmount !== null && vatAmount !== null && grossAmount !== null && netAmount + vatAmount !== grossAmount) errors.push(error("grossAmount", "Net amount plus VAT amount must equal gross amount exactly."));
	...
	if (supplier && invoiceNumber) {
		if (duplicateCandidates.find((candidate) => candidate.id !== invoiceId && normalizeText(candidate.supplierName) === supplier && normalizeText(candidate.invoiceNumber) === invoiceNumber)) errors.push(error("duplicate", "Duplicate invoice number for this supplier was found in the current workspace."));
	}
	return errors;
}
//#endregion
//#region lib/services/exact-token-crypto.ts
...
async function exactCryptoKey() {
	const secret = process.env.OAUTH_TOKEN_ENCRYPTION_KEY || process.env.EXACT_TOKEN_ENCRYPTION_KEY;
	if (!secret) throw new Error("OAUTH_TOKEN_ENCRYPTION_KEY or EXACT_TOKEN_ENCRYPTION_KEY is required for real OAuth token storage.");
	const digest = await crypto.subtle.digest("SHA-256", encoder.encode(secret));
	return crypto.subtle.importKey("raw", digest, "AES-GCM", false, ["encrypt", "decrypt"]);
}
async function encryptExactSecret(value) {
...
var encryptOAuthSecret = encryptExactSecret;
async function decryptExactSecret(value) {
	if (value.startsWith("mock-")) return value;
	const [version, iv, encrypted] = value.split(".");
	if (version !== "v1" || !iv || !encrypted) throw new Error("Stored Exact Online token is not in a supported format.");
	const key = await exactCryptoKey();
	const decrypted = await crypto.subtle.decrypt({
		name: "AES-GCM",
		iv: base64UrlDecode(iv)
	}, key, base64UrlDecode(encrypted));
	return decoder.decode(decrypted);
}
var decryptOAuthSecret = decryptExactSecret;
async function signExactState(payload) {
	const secret = process.env.OAUTH_STATE_SECRET || process.env.EXACT_OAUTH_STATE_SECRET || process.env.OAUTH_TOKEN_ENCRYPTION_KEY || process.env.EXACT_TOKEN_ENCRYPTION_KEY;
	if (!secret) throw new Error("OAUTH_STATE_SECRET, EXACT_OAUTH_STATE_SECRET, OAUTH_TOKEN_ENCRYPTION_KEY, or EXACT_TOKEN_ENCRYPTION_KEY is required for real OAuth.");
...
function encodeExactStatePayload(payload) {
	return base64UrlEncode(encoder.encode(JSON.stringify(payload)));
}
function decodeExactStatePayload(value) {
	return JSON.parse(decoder.decode(base64UrlDecode(value)));
}
//#endregion
//#region lib/services/exact-api-client.ts
var exactDefaultBaseUrl = "https://start.exactonline.nl";
var exactMasterDataResourcePatterns = [
	/\/crm\/accounts(?:[/?(]|$)/i,
...
function exactIntegrationMode() {
	const explicit = process.env.EXACT_ONLINE_MODE?.toLowerCase();
	if (explicit === "real" || explicit === "mock") return explicit;
	return process.env.EXACT_ONLINE_CLIENT_ID && process.env.EXACT_ONLINE_CLIENT_SECRET ? "real" : "mock";
}
function isRealExactMode() {
	return exactIntegrationMode() === "real";
}
function isMockExactConnection(connection) {
	return !connection || connection.accessTokenCiphertext.startsWith("mock-") || connection.refreshTokenCiphertext.startsWith("mock-");
}
function isExactMasterDataResource(pathOrUrl) {
	const path = pathOrUrl.startsWith("http") || pathOrUrl.startsWith("https") ? new URL(pathOrUrl).pathname : pathOrUrl;
	return exactMasterDataResourcePatterns.some((pattern) => pattern.test(path));
}
function assertExactMasterDataReadOnlyRequest(method, pathOrUrl) {
	const normalizedMethod = method.toUpperCase();
	if (normalizedMethod !== "GET" && normalizedMethod !== "HEAD" && isExactMasterDataResource(pathOrUrl)) throw new Error(`Blocked ${normalizedMethod} request to Exact Online master data resource ${pathOrUrl}. INTO is read-only for Exact master data.`);
}
function exactConfig() {
	...
	const redirectUri = process.env.EXACT_ONLINE_REDIRECT_URI || `${process.env.APP_URL || "http://localhost:3000"}/api/exact/callback`;
	if (isRealExactMode() && (!clientId || !clientSecret || !redirectUri)) throw new Error("EXACT_ONLINE_CLIENT_ID, EXACT_ONLINE_CLIENT_SECRET, and EXACT_ONLINE_REDIRECT_URI are required for real Exact Online.");
	return {
		baseUrl,
		clientId,
		clientSecret,
...
async function createExactOAuthState(userId) {
	const payload = encodeExactStatePayload({
		userId,
		nonce: createId("exact_state"),
		issuedAt: Date.now()
	});
	return `${payload}.${await signExactState(payload)}`;
}
async function verifyExactOAuthState(state) {
	const [payload, signature] = state.split(".");
	if (!payload || !signature) throw new Error("Exact OAuth state is missing or invalid.");
	if (signature !== await signExactState(payload)) throw new Error("Exact OAuth state signature is invalid.");
	const decoded = decodeExactStatePayload(payload);
	if (Date.now() - decoded.issuedAt > 900 * 1e3) throw new Error("Exact OAuth state has expired. Start the connection again.");
	return decoded;
}
async function createRealExactAuthorizationUrl(userId) {
	const { baseUrl, clientId, redirectUri } = exactConfig();
	const state = await createExactOAuthState(userId);
	return {
		authorizationUrl: `${baseUrl}/api/oauth2/auth?${new URLSearchParams({
			client_id: clientId,
...
async function requestExactToken(params) {
...
function scopesFromTokenResponse(token) {
	return (token.scope || "").split(/[ ,]+/).map((scope) => scope.trim()).filter(Boolean);
}
async function connectionFromToken(userId, token, divisionCode = "") {
	const now = /* @__PURE__ */ new Date();
	const expiresInSeconds = token.expires_in ?? 600;
	if (!token.refresh_token) throw new Error("Exact OAuth response did not include a refresh token.");
	return {
		id: createId("exact"),
		userId,
		divisionCode,
		...
		expiresAt: new Date(now.getTime() + expiresInSeconds * 1e3).toISOString(),
		scopes: scopesFromTokenResponse(token),
		createdAt: now.toISOString(),
		updatedAt: now.toISOString()
	};
}
async function exchangeExactAuthorizationCode(userId, code) {
	const connection = await connectionFromToken(userId, await requestExactToken({
...
async function refreshRealExactConnection(connection) {
	const token = await requestExactToken({
		grant_type: "refresh_token",
		refresh_token: await decryptExactSecret(connection.refreshTokenCiphertext)
	});
	const now = /* @__PURE__ */ new Date();
	const expiresInSeconds = token.expires_in ?? 600;
	return {
		...connection,
		accessTokenCiphertext: await encryptExactSecret(token.access_token),
		refreshTokenCiphertext: token.refresh_token ? await encryptExactSecret(token.refresh_token) : connection.refreshTokenCiphertext,
		expiresAt: new Date(now.getTime() + expiresInSeconds * 1e3).toISOString(),
		scopes: scopesFromTokenResponse(token).length ? scopesFromTokenResponse(token) : connection.scopes,
		updatedAt: now.toISOString()
	};
}
async function exactAccessToken(connection) {
...
function odataNext(payload) {
	return payload.d?.__next;
}
async function fetchExactJson(connection, pathOrUrl) {
	...
	const payload = await response.json().catch(() => ({}));
	if (!response.ok) {
		const detail = payload.error?.message?.value || payload.error_description || payload.error || response.statusText;
		throw new Error(`Exact API request failed for ${pathOrUrl}: ${detail}`);
	}
	return payload;
}
async function fetchExactOData(connection, divisionCode, relativePath, options = {}) {
	const maxPages = options.maxPages ?? 10;
	let pathOrUrl = `/api/v1/${divisionCode}${relativePath}`;
	const results = [];
	for (let page = 0; pathOrUrl && page < maxPages; page += 1) {
		const payload = await fetchExactJson(connection, pathOrUrl);
		results.push(...odataResults(payload));
		pathOrUrl = odataNext(payload) ?? "";
	}
	return results;
}
async function fetchFirstAvailable(connection, divisionCode, paths, options = {}) {
	const failures = [];
	for (const path of paths) try {
		return await fetchExactOData(connection, divisionCode, path, options);
	} catch (error) {
...
function booleanValueOf(record, keys, fallback = true) {
	for (const key of keys) {
		const value = record[key];
...
function numberValueOf(record, keys) {
	for (const key of keys) {
		const value = record[key];
...
function mapSupplier(value) {
	const record = normalizeRecord(value);
	const code = valueOf(record, [
		"Code",
		"AccountCode",
		"SupplierCode",
		"ID"
	]);
	const id = valueOf(record, [
		"ID",
		"Id",
		"AccountID",
		"Account",
	...
	return {
		...
		paymentConditionCode,
		paymentConditionLabel: valueOf(record, ["PaymentConditionDescription", "PaymentConditionLabel"]) || paymentConditionCode,
		defaultGlAccount,
		defaultGlAccountName: valueOf(record, ["GLAccountPurchaseDescription", "DefaultGLAccountName"]) || defaultGlAccount,
		defaultCostCentre: valueOf(record, [
			"Costcenter",
			"CostCenter",
			"CostCentre"
		]),
		defaultCostUnit: valueOf(record, ["Costunit", "CostUnit"]),
		isInBodyEntity: /inbody/i.test(valueOf(record, [
			"Name",
			"AccountName",
			"SupplierName"
		]))
...
function mapPaymentCondition(value) {
	const record = normalizeRecord(value);
	const code = valueOf(record, [
		"Code",
		"ID",
	...
	return {
		code,
		label: valueOf(record, [
			"Description",
			"Name",
			"Label"
		]) || code,
		days: numberValueOf(record, [
			"Days",
			"PaymentDays",
			"TermInDays"
		]),
...
function mapJournal(value) {
	...
	return {
		code,
		description,
		type: /purchase|inkoop/.test(`${typeValue} ${description.toLowerCase()}`) ? "purchase" : "general",
		isActive: booleanValueOf(record, ["IsActive", "Active"], true)
	};
}
function mapGlAccount(value) {
	const record = normalizeRecord(value);
	const code = valueOf(record, [
		"Code",
		"GLAccount",
...
function mapVatCode(value) {
	...
	return {
		code,
		description: valueOf(record, ["Description", "Name"]) || code,
		percentage: numberValueOf(record, [
			"Percentage",
			"VATPercentage",
			"Rate"
		]) ?? 0,
		type: /sales/i.test(valueOf(record, [
			"Type",
			"VATType",
			"TaxType"
		])) ? "sales" : "purchase",
...
function mapHistory(value) {
	...
	if (!supplierAccountId || !glAccount) return null;
	return {
		id: valueOf(record, [
			"ID",
			"EntryID",
		...
		costCentre: valueOf(record, [
			"Costcenter",
			"CostCenter",
			"CostCentre"
		]),
		costUnit: valueOf(record, ["Costunit", "CostUnit"]),
		accrualFrom: valueOf(record, [
			"From",
			"DateFrom",
			"AccrualFrom"
		]),
		accrualTo: valueOf(record, [
			"To",
			"DateTo",
			"AccrualTo"
		])
...
async function fetchExactCurrentDivision(connection) {
	const [me] = odataResults(await fetchExactJson(connection, "/api/v1/current/Me"));
	const division = valueOf(normalizeRecord(me), [
		"CurrentDivision",
		"Division",
		"DivisionCode"
	]);
	if (!division) throw new Error("Exact Online did not return the current division.");
	return division;
}
async function syncRealExactMasterData(connection) {
	const divisionCode = connection.divisionCode || await fetchExactCurrentDivision(connection);
	const syncedAt = /* @__PURE__ */ new Date();
	const [suppliersRaw, paymentConditionsRaw, journalsRaw, glAccountsRaw, costCentersRaw, costUnitsRaw, vatCodesRaw, historicalRaw] = await Promise.all([
		fetchFirstAvailable(connection, divisionCode, ["/crm/Accounts?$filter=IsSupplier eq true&$top=500", "/crm/Accounts?$top=500"]),
		fetchFirstAvailable(connection, divisionCode, ["/crm/PaymentConditions?$top=500", "/financial/PaymentConditions?$top=500"]),
		fetchFirstAvailable(connection, divisionCode, ["/financial/Journals?$top=500"]),
		fetchFirstAvailable(connection, divisionCode, ["/financial/GLAccounts?$top=500"]),
		fetchFirstAvailable(connection, divisionCode, ["/hrm/Costcenters?$top=500", "/financial/Costcenters?$top=500"], { optional: true }),
		fetchFirstAvailable(connection, divisionCode, ["/hrm/Costunits?$top=500", "/financial/Costunits?$top=500"], { optional: true }),
		fetchFirstAvailable(connection, divisionCode, ["/vat/VATCodes?$top=500", "/financial/VATCodes?$top=500"]),
		fetchFirstAvailable(connection, divisionCode, ["/purchaseentry/PurchaseEntryLines?$top=500", "/purchaseentry/PurchaseEntries?$top=500"], {
			optional: true,
			maxPages: 3
		})
	]);
	const historicalPurchaseBookings = historicalRaw.map(mapHistory).filter((item) => Boolean(item));
	return {
		source: "exact-online",
		divisionCode,
		lastSyncedAt: syncedAt.toISOString(),
		staleAfter: new Date(syncedAt.getTime() + 1800 * 1e3).toISOString(),
		suppliers: suppliersRaw.map(mapSupplier).filter((supplier) => supplier.name),
		paymentConditions: paymentConditionsRaw.map(mapPaymentCondition).filter((condition) => condition.code),
		...
		costCenters: costCentersRaw.map(mapCostCenter).filter((costCenter) => costCenter.code),
...
async function createRealExactPurchaseBooking(connection, invoice, masterData) {
	if (process.env.EXACT_ONLINE_ENABLE_REAL_BOOKING !== "true") throw new Error("Real Exact Online booking is disabled. Set EXACT_ONLINE_ENABLE_REAL_BOOKING=true only after validating the purchase-entry payload with your Exact Online division.");
	throw new Error("Real Exact Online purchase-entry posting adapter is not implemented yet. INTO is connected to Exact for OAuth and master-data sync.");
}
//#endregion
//#region lib/services/exact-master-data-service.ts
var glAccounts = {
	office: {
		code: "4400",
...
async function syncExactMasterData(connection) {
	if (!connection || connection.status !== "connected") throw new Error("Connect Exact Online before syncing master data.");
	if (isRealExactMode() && !isMockExactConnection(connection)) return syncRealExactMasterData(connection);
	return createMockExactMasterData(connection.divisionCode);
}
function createMockExactMasterData(divisionCode = "123456") {
	const syncedAt = /* @__PURE__ */ new Date();
	return {
		source: "exact-online",
		divisionCode,
		...
		vatCodes: [
			...
			{
				code: "7",
				description: "EU acquisition VAT",
				percentage: 0,
				type: "purchase",
				isActive: true
			},
			{
				code: "8",
				description: "Non-EU acquisition VAT",
				percentage: 0,
				type: "purchase",
				isActive: true
			}
...
function isExactMasterDataStale(cache) {
	return !cache || new Date(cache.staleAfter).getTime() <= Date.now();
}
//#endregion
//#region lib/services/storage-service.ts
...
var globalFileStore = globalThis;
function fileStore() {
	if (!globalFileStore.__INTO_FILE_STORE) globalFileStore.__INTO_FILE_STORE = /* @__PURE__ */ new Map();
...
function contentTypeFor(fileName, fileType = "") {
	if (fileType && fileType !== "application/octet-stream") return fileType;
	const extension = getFileExtension(fileName);
	if (extension === "pdf") return "application/pdf";
	if (extension === "jpg" || extension === "jpeg") return "image/jpeg";
	if (extension === "png") return "image/png";
	if (extension === "xml" || extension === "ubl") return "application/xml";
	return "application/octet-stream";
}
async function storeInvoiceFile(file) {
	const storedFile = {
...
function storeMockInvoiceFile(input) {
...
function getStoredInvoiceFile(storageKey) {
	return fileStore().get(storageKey) ?? null;
}
//#endregion
//#region lib/services/purchase-journal-intelligence.ts
var companyConfig = {
	confidenceThreshold: .86,
	companyVatNumber: "NL857017263B01",
	defaultJournal: "60",
	intercompanyJournal: "61",
...
function exactSuppliers(exactMasterData) {
	return exactMasterData?.suppliers.filter((supplier) => supplier.name) ?? [];
}
function exactGlAccount(exactMasterData, code) {
	return exactMasterData?.glAccounts.find((account) => account.code === code && account.isActive);
}
function exactVatCode(exactMasterData, code) {
	return exactMasterData?.vatCodes.find((vatCode) => vatCode.code === code && vatCode.type === "purchase" && vatCode.isActive);
}
function exactCostCenter(exactMasterData, code) {
	if (!code) return;
	return exactMasterData?.costCenters.find((costCenter) => costCenter.code === code && costCenter.isActive);
}
function exactCostUnit(exactMasterData, code) {
	if (!code) return;
	return exactMasterData?.costUnits.find((costUnit) => costUnit.code === code && costUnit.isActive);
}
function exactHistoricalBooking(exactMasterData, supplier, description) {
	if (!supplier) return;
	const key = descriptionKey(description);
	return exactMasterData?.historicalPurchaseBookings.find((booking) => booking.supplierAccountId === supplier.id && (booking.descriptionKey === key || key.includes(booking.descriptionKey)));
}
function selectedSupplierFromBooking(invoice, exactMasterData) {
...
function resolveSupplier(invoice, allInvoices, learning, exactMasterData) {
	...
	if (!exactMasterData) return {
		selectedAccountId: void 0,
		selectedAccountCode: void 0,
		selectedAccountName: void 0,
		matchConfidence: 0,
	...
	for (const decision of learning.supplierSelections) {
		if (!identities.includes(decision.supplierIdentity)) continue;
		const account = suppliers.find((supplier) => supplier.id === decision.accountId);
		if (account) mergeCandidate(candidates, account, 1, "Learned decision", "Matched a previously approved supplier resolution.");
	}
	const dataVat = normalizedVat(data.supplierVatNumber);
	if (dataVat) {
		for (const account of suppliers) if (normalizedVat(account.vatNumber) === dataVat) mergeCandidate(candidates, account, .99, "VAT number", "VAT number matched Exact supplier master data.");
	}
	const dataIban = normalizeText(data.iban);
	if (dataIban) {
		for (const account of suppliers) if (normalizeText(account.iban) === dataIban) mergeCandidate(candidates, account, .97, "IBAN", "IBAN matched Exact supplier master data.");
	}
	for (const previous of allInvoices) {
		...
		if (previousSupplier && normalizeText(previous.extractedData.supplierName) === normalizeText(data.supplierName)) mergeCandidate(candidates, previousSupplier, .94, "Exact history", "Same supplier name was previously booked to this Exact account.");
	}
	const invoiceDescriptionKey = descriptionKey(humanDescription(data));
	for (const historical of exactMasterData.historicalPurchaseBookings) if (invoiceDescriptionKey && (invoiceDescriptionKey === historical.descriptionKey || invoiceDescriptionKey.includes(historical.descriptionKey))) {
		const account = suppliers.find((supplier) => supplier.id === historical.supplierAccountId);
		if (account) mergeCandidate(candidates, account, .93, "Exact history", "Similar invoice description matched historical Exact purchase bookings.");
	}
	for (const account of suppliers) {
		const score = nameSimilarity(data.supplierName, account.name);
		if (score >= .55) mergeCandidate(candidates, account, roundMoney(.5 + score * .35), "Name similarity", "Supplier name is similar to the Exact account name.");
	}
	const sorted = [...candidates.values()].sort((a, b) => b.confidence - a.confidence);
	const learnedSelection = sorted.find((candidate) => candidate.method === "Learned decision" && candidate.confidence >= companyConfig.confidenceThreshold);
	const strong = sorted.filter((candidate) => candidate.confidence >= companyConfig.confidenceThreshold);
	const reviewRequired = learnedSelection ? false : strong.length !== 1;
	const selected = learnedSelection ?? (reviewRequired ? void 0 : strong[0]);
	return {
		...
		reviewRequired,
		candidates: sorted,
		reasoning: sorted.length > 0 ? sorted.flatMap((candidate) => candidate.reasoning) : ["No supplier account reached the configured confidence threshold."]
	};
}
function yearMonth(dateValue) {
	if (!isValidIsoDate(dateValue)) return {
		year: companyConfig.currentMockOpenYear,
		month: companyConfig.firstMockOpenPeriod
	};
...
function humanDescription(data) {
	return (data.expenseDescription || data.lineItems[0]?.description || data.rawText?.match(/(?:for|description)\s+([a-z0-9 ]{4,50})/i)?.[1] || "Invoice expenses").trim().replace(/\s+/g, " ");
}
function headerDescription(data) {
	return `${monthLabel(data.invoiceDate)}.${humanDescription(data)}`;
}
function lineDescription(data, lineDescriptionText) {
...
function paymentConditionFor(supplier, data, exactMasterData) {
	const extracted = (data.paymentTerms ?? "").trim();
	const condition = exactMasterData?.paymentConditions.find((item) => item.code === supplier?.paymentConditionCode && item.isActive);
	const conditionCode = condition?.code ?? supplier?.paymentConditionCode ?? "";
	const conditionLabel = condition?.label ?? supplier?.paymentConditionLabel ?? "Unknown Exact payment condition";
	const mismatch = Boolean(extracted) && normalizeText(extracted) !== normalizeText(conditionLabel);
	...
	return {
		...
		mismatch,
		missing,
		confidence: missing ? 0 : mismatch ? .52 : extracted ? .94 : .78
	};
}
function isOpenPeriod(year, period) {
	if (year > companyConfig.currentMockOpenYear) return true;
	if (year < companyConfig.currentMockOpenYear) return false;
	return period >= companyConfig.firstMockOpenPeriod;
}
function firstOpenPeriodOnOrAfter(year, period) {
	let nextYear = year;
	let nextPeriod = period;
	for (let index = 0; index < 36; index += 1) {
		if (isOpenPeriod(nextYear, nextPeriod)) return {
			year: nextYear,
			period: nextPeriod
		};
		nextPeriod += 1;
...
function determineFinancialPeriod(data) {
	const invoiceMonth = yearMonth(data.invoiceDate);
	const open = firstOpenPeriodOnOrAfter(invoiceMonth.year, invoiceMonth.month);
	const adjusted = open.year !== invoiceMonth.year || open.period !== invoiceMonth.month;
	return {
		financialYear: open.year,
		period: open.period,
		adjusted,
		log: adjusted ? `Invoice period ${invoiceMonth.year}/${invoiceMonth.month} is closed in mock Exact, so INTO moved the booking to ${open.year}/${open.period}.` : "Invoice date maps to an open Exact period."
	};
}
function daysBetween(left, right) {
	if (!isValidIsoDate(left) || !isValidIsoDate(right)) return 0;
...
function monthSpan(start, end) {
	...
	const startDate = /* @__PURE__ */ new Date(`${start}T00:00:00.000Z`);
	const endDate = /* @__PURE__ */ new Date(`${end}T00:00:00.000Z`);
...
function firstDayOfPeriod(year, period) {
	return `${year}-${String(period).padStart(2, "0")}-01`;
}
function determineAccrual(data) {
	if (!isValidIsoDate(data.serviceStartDate) || !isValidIsoDate(data.serviceEndDate)) return {
		from: "",
		to: "",
		benefitStartDate: void 0,
		benefitEndDate: void 0,
		reason: void 0
	};
	...
	const accrualKeyword = /subscription|insurance|maintenance|flight|hotel|event/i.test(`${humanDescription(data)} ${data.rawText ?? ""}`);
	if (periodMonths <= 3 && !paidBeforeBenefit && !accrualKeyword) return {
		from: "",
		to: "",
		benefitStartDate: void 0,
		benefitEndDate: void 0,
	...
	const reason = periodMonths > 3 ? `Service period spans ${periodMonths} months.` : paidBeforeBenefit ? "Expense is paid before the benefit period." : "Expense category normally requires accrual review.";
	return {
		from,
		to,
...
function selectedSupplierAccount(candidates, exactMasterData) {
	return exactSuppliers(exactMasterData).find((account) => account.id === candidates.selectedAccountId);
}
function suggestGlAccount(supplier, data, lineDescriptionText, learning, exactMasterData) {
	const text = normalizeText(`${supplier?.name ?? ""} ${data.expenseDescription} ${lineDescriptionText} ${data.rawText ?? ""}`);
	const key = descriptionKey(lineDescriptionText || humanDescription(data));
	const suggestionForCode = (code, confidence, reason) => {
	...
	const learned = supplier ? learning.glAccountSelections.find((decision) => decision.supplierAccountId === supplier.id && decision.descriptionKey === key) : void 0;
	if (learned) return suggestionForCode(learned.glAccount, .97, "User-approved mapping from a previous invoice.");
	if (supplier?.defaultGlAccount) return suggestionForCode(supplier.defaultGlAccount, supplier.isInBodyEntity ? .95 : .9, "Supplier default G/L account from Exact master data.");
	const historical = exactHistoricalBooking(exactMasterData, supplier, lineDescriptionText || humanDescription(data));
	if (historical) return suggestionForCode(historical.glAccount, .96, "Matched historical Exact purchase booking for this supplier and description.");
	if (/google|workspace|software|saas|subscription/.test(text)) return suggestionForCode(glRuleCodes.software, .89, "Matched software subscription pattern.");
	if (/booking|hotel|restaurant|lodging/.test(text)) return suggestionForCode(glRuleCodes.hotel, .89, "Matched travel hotel pattern.");
	if (/klm|flight|airline|air travel/.test(text)) return suggestionForCode(glRuleCodes.airTravel, .9, "Matched air travel pattern.");
	if (/insurance|policy/.test(text)) return suggestionForCode(glRuleCodes.insurance, .9, "Matched insurance pattern.");
	if (/bank fee|payment fee|stripe/.test(text)) return suggestionForCode(glRuleCodes.bank, .88, "Matched bank fee pattern.");
	if (/office|supplies|furniture/.test(text)) return suggestionForCode(glRuleCodes.office, .88, "Matched office supplies pattern.");
	return suggestionForCode(glRuleCodes.services, .62, "Fallback to professional services because no strong pattern matched.");
}
function isDutchSupplier(supplier, data) {
	return (supplier?.country || data.supplierCountry || "").toUpperCase() === "NL";
}
function supplierCountry(supplier, data) {
	return (supplier?.country || data.supplierCountry || "").toUpperCase();
}
function vatRateFor(line) {
	if (!line.netAmount) return 0;
	return roundMoney(line.vatAmount / line.netAmount);
}
function suggestVatCode(supplier, data, lineDescriptionText, line, exactMasterData) {
	const country = supplierCountry(supplier, data);
	const supplierVat = normalizedVat(data.supplierVatNumber || supplier?.vatNumber || "");
	const companyVatPresent = normalizedVat(data.companyVatNumber) === normalizedVat(companyConfig.companyVatNumber);
	...
	const text = normalizeText(`${supplier?.name ?? data.supplierName} ${data.expenseDescription} ${lineDescriptionText} ${data.rawText ?? ""}`);
	const safeFallbackPattern = /flight|airline|eurostar|bank fee|insurance|stamp|postnl|hotel|restaurant|exempt|non-deductible/.test(text);
	const zeroVat = Math.abs(line.vatAmount) < .005 || Math.abs(rate) < .005;
	const suggestionForCode = (code, confidence, percentage, amountMode, reasoning) => {
		...
		if (!exactCode) return {
			...
			percentage,
			amountMode,
			reasoning: [...reasoning, `Expected purchase VAT code ${code} is missing or inactive in Exact.`]
		};
		return {
			code,
	...
	if (companyVatPresent && supplierNonDutch && euVatCountries.has(country) && (data.reverseChargeMentioned || data.intraCommunityMentioned)) return suggestionForCode("7", .95, 0, "vat_inclusive", [
	...
	if (companyVatPresent && supplierOutsideEu && zeroVat) return suggestionForCode("8", .93, 0, "vat_inclusive", [
		"Company VAT number is present.",
		"Supplier is outside EU VAT regions.",
		"Invoice shows 0% VAT and no reverse-charge wording was detected."
	]);
	if (safeFallbackPattern) return suggestionForCode("6", .9, 0, "vat_inclusive", ["Expense category belongs to the safe VAT code 6 fallback list."]);
	if (isDutchSupplier(supplier, data) && Math.abs(rate - .21) < .001) return suggestionForCode("4", .94, 21, "vat_excluded", ["Dutch supplier with VAT amount at 21%."]);
	if (isDutchSupplier(supplier, data) && Math.abs(rate - .09) < .001) return suggestionForCode("5", .94, 9, "vat_excluded", ["Dutch supplier with VAT amount at 9%."]);
	return suggestionForCode("6", zeroVat ? .88 : .68, 0, "vat_inclusive", [zeroVat ? "Invoice shows 0% VAT, so code 6 is the safe fallback." : "VAT classification confidence is low, so INTO selected safe fallback code 6."]);
}
function yourRefValue(data) {
	return (data.referenceCode ?? "").trim() || (data.invoiceNumber ?? "").trim();
}
function isYourRefUnique(invoice, allInvoices, supplier) {
	...
	return !allInvoices.some((candidate) => {
		...
		const sameSupplierAccount = supplier && candidate.purchaseJournal?.supplierResolution.selectedAccountId === supplier.id;
		const sameSupplierName = normalizeText(candidate.extractedData.supplierName) === normalizeText(invoice.extractedData.supplierName);
		return normalizeText(yourRefValue(candidate.extractedData)) === reference && (sameSupplierAccount || sameSupplierName);
	});
}
function sourceLines(data) {
	if (data.lineItems.length > 0) return data.lineItems.map((item) => ({
		id: item.id,
	...
	return [{
		...
		vatRate: data.netAmount && data.vatAmount ? roundMoney(data.vatAmount / data.netAmount) : 0
	}];
}
function costSelection(supplier, glAccount, learning, exactMasterData, lineDescriptionText) {
	const historical = exactHistoricalBooking(exactMasterData, supplier, lineDescriptionText);
	const learnedCentre = supplier ? learning.costCentreSelections.find((decision) => decision.supplierAccountId === supplier.id && decision.glAccount === glAccount) : void 0;
	const learnedUnit = supplier ? learning.costUnitSelections.find((decision) => decision.supplierAccountId === supplier.id && decision.glAccount === glAccount) : void 0;
	const centreCode = learnedCentre?.costCentre ?? historical?.costCentre ?? supplier?.defaultCostCentre ?? "";
	const unitCode = learnedUnit?.costUnit ?? historical?.costUnit ?? supplier?.defaultCostUnit ?? "";
	...
	return {
		...
		costCentreConfidence: learnedCentre ? centre ? .96 : 0 : historical?.costCentre ? centre ? .92 : 0 : supplier?.defaultCostCentre ? centre ? .88 : 0 : .9,
		costUnit: unit?.code ?? "",
		costUnitConfidence: learnedUnit ? unit ? .96 : 0 : historical?.costUnit ? unit ? .92 : 0 : supplier?.defaultCostUnit ? unit ? .88 : 0 : .9
	};
}
function buildLines(supplier, data, learning, exactMasterData) {
	const accrual = determineAccrual(data);
	const lines = sourceLines(data).map((line) => {
		const gl = suggestGlAccount(supplier, data, line.description, learning, exactMasterData);
		const vat = suggestVatCode(supplier, data, line.description, line, exactMasterData);
		const costs = costSelection(supplier, gl.account, learning, exactMasterData, line.description);
		const amount = vat.amountMode === "vat_inclusive" ? line.grossAmount : line.netAmount;
		const vatAmount = vat.amountMode === "vat_inclusive" ? 0 : line.vatAmount;
		const reviewRequired = gl.confidence < companyConfig.confidenceThreshold || vat.confidence < companyConfig.confidenceThreshold || costs.costCentreConfidence < companyConfig.confidenceThreshold || costs.costUnitConfidence < companyConfig.confidenceThreshold;
		return {
			id: createId("pj_line"),
	...
	const difference = roundMoney((data.grossAmount ?? 0) - roundMoney(lines.reduce((sum, line) => sum + line.amount + line.vatAmount, 0)));
	if (lines.length > 0 && Math.abs(difference) > 0) {
		...
		lastLine.reasoning.push(`Adjusted booking line by ${difference.toFixed(2)} so the booked total exactly matches the invoice total.`);
	}
	return lines;
}
function confidenceScores(supplierConfidence, paymentConfidence, lines) {
	const glAccount = lines.length > 0 ? Math.min(...lines.map((line) => line.glConfidence)) : 0;
	const vatCode = lines.length > 0 ? Math.min(...lines.map((line) => line.vatConfidence)) : 0;
	const costCentre = lines.length > 0 ? Math.min(...lines.map((line) => line.costCentreConfidence)) : .9;
	const costUnit = lines.length > 0 ? Math.min(...lines.map((line) => line.costUnitConfidence)) : .9;
	const overall = Math.min(supplierConfidence || 0, glAccount || 0, vatCode || 0, costCentre || 0, costUnit || 0, paymentConfidence || 0);
	return {
...
function totalsFor(lines, invoiceGross) {
...
function exactMasterDataIssues(exactMasterData, supplier, journal, lines, paymentMissing) {
	if (!exactMasterData) return ["Exact master data has not been synced."];
	const issues = [];
	const requiredJournals = [companyConfig.defaultJournal, companyConfig.intercompanyJournal];
	for (const journalCode of requiredJournals) if (!exactMasterData.journals.find((item) => item.code === journalCode && item.type === "purchase" && item.isActive)) issues.push(`Exact purchase journal ${journalCode} is missing or inactive.`);
	if (!exactMasterData.journals.some((item) => item.code === journal && item.type === "purchase" && item.isActive)) issues.push(`Selected Exact purchase journal ${journal} is not available.`);
	if (supplier && !exactMasterData.suppliers.some((item) => item.id === supplier.id)) issues.push("Selected supplier is not present in the synced Exact supplier data.");
	if (paymentMissing) issues.push("Supplier default payment condition is missing in Exact.");
	for (const line of lines) {
		if (!exactGlAccount(exactMasterData, line.finalSelectedAccount)) issues.push(`G/L account ${line.finalSelectedAccount} is missing in Exact.`);
		if (!exactVatCode(exactMasterData, line.vatCode)) issues.push(`Purchase VAT code ${line.vatCode} is missing in Exact.`);
		if (line.costCentre && !exactCostCenter(exactMasterData, line.costCentre)) issues.push(`Cost center ${line.costCentre} is missing in Exact.`);
		if (line.costUnit && !exactCostUnit(exactMasterData, line.costUnit)) issues.push(`Cost unit ${line.costUnit} is missing in Exact.`);
	}
	return [...new Set(issues)];
}
function createInitialLearningStore() {
	return {
		supplierSelections: [],
		glAccountSelections: [],
		vatCodeSelections: [],
...
function generatePurchaseJournalBooking(invoice, allInvoices, learning, exactMasterData) {
	const data = invoice.extractedData;
	const supplierResolution = resolveSupplier(invoice, allInvoices, learning, exactMasterData);
	...
	const journal = supplier?.isInBodyEntity ? companyConfig.intercompanyJournal : companyConfig.defaultJournal;
	const confidence = confidenceScores(supplierResolution.matchConfidence, payment.confidence, lines);
	const masterDataIssues = exactMasterDataIssues(exactMasterData, supplier, journal, lines, payment.missing);
	...
	const criticalReasons = [
		...
		Math.abs(totals.difference) > .005 ? "Booking line total does not match the invoice total." : ""
	].filter(Boolean);
	const reviewableReasons = [
		payment.mismatch ? "Invoice payment terms do not match the Exact supplier default payment condition." : "",
		lines.some((line) => line.reviewRequired) ? "One or more G/L or VAT decisions are below the configured confidence threshold." : "",
		confidence.overall < companyConfig.confidenceThreshold ? "Overall booking confidence is below the configured threshold." : ""
	].filter(Boolean);
	const reviewReasons = [...criticalReasons, ...userApproved ? [] : reviewableReasons];
	return {
		attachmentRequired: true,
		attachmentPresent,
		attachmentStorageKey: invoice.storageKey || void 0,
		...
		journal,
		journalReason: supplier?.isInBodyEntity ? "Supplier is configured as an InBody group company." : "Default purchase journal.",
		...
		autoBookAllowed: attachmentPresent && criticalReasons.length === 0 && (!reviewableReasons.length || userApproved),
		userApproved,
		reviewRequired: reviewReasons.length > 0,
		reviewReasons,
		reasoningLog: [
			`Supplier match method: ${supplierResolution.method}.`,
			`Journal ${journal} selected: ${supplier?.isInBodyEntity ? "InBody group company exception." : "standard purchase booking."}`,
			period.log,
			exactMasterData ? `Exact master data synced at ${exactMasterData.lastSyncedAt}.` : "Exact master data is not synced.",
			...lines.flatMap((line) => line.reasoning)
		],
		learningSummary: ["Supplier, G/L, VAT, cost centre, cost unit, and accrual decisions are captured for future learning once approved or booked."]
	};
}
function purchaseError(field, message, severity = "error") {
	return {
		id: createId("val"),
		field,
		message,
...
function isExactMasterDataReason(reason) {
	return reason.startsWith("Exact master data") || reason.startsWith("Exact purchase journal") || reason.startsWith("Selected Exact") || reason.includes("missing in Exact") || reason.includes("missing or inactive in Exact") || reason.includes("not present in the synced Exact");
}
function purchaseJournalValidationErrors(booking) {
	const errors = [];
	if (!booking.attachmentPresent) errors.push(purchaseError("attachment", "Original invoice attachment is required and must be uploaded to Exact with the booking."));
	if (booking.supplierResolution.reviewRequired) errors.push(purchaseError("supplier", "Supplier Review Required: select one of the matching Exact supplier accounts."));
	if (!booking.yourRef) errors.push(purchaseError("yourRef", "Your ref. is required and must contain the invoice number or invoice reference."));
	if (!booking.yourRefUnique) errors.push(purchaseError("yourRef", "Your ref. must be unique for the selected supplier to prevent duplicate bookings."));
	if (booking.paymentConditionMismatch && !booking.userApproved) errors.push(purchaseError("paymentCondition", "Payment Condition Review Required: invoice payment terms differ from the Exact supplier default."));
	if (booking.reviewRequired) for (const reason of booking.reviewReasons) errors.push(purchaseError(isExactMasterDataReason(reason) ? "exactMasterData" : "purchaseJournal", reason));
	return errors;
}
function statusFromPurchaseJournal(baseValidationErrors, booking) {
	if (!booking.attachmentPresent) return "Attachment Missing";
	if (baseValidationErrors.some((item) => item.severity === "error")) return "Validation Failed";
	if (booking.reviewReasons.some(isExactMasterDataReason)) return "Booking Intelligence Review Required";
	if (booking.supplierResolution.reviewRequired) return "Supplier Review Required";
	if (booking.paymentConditionMismatch && !booking.userApproved) return "Payment Condition Review Required";
	if (booking.reviewRequired) return "Booking Intelligence Review Required";
	return "Ready to Book";
}
function rememberDecisionsFromInvoice(invoice, learning) {
	const booking = invoice.purchaseJournal;
	if (!booking?.supplierResolution.selectedAccountId) return;
	...
	if (!learning.supplierSelections.some((decision) => decision.supplierIdentity === supplierIdentity && decision.accountId === supplierAccountId)) learning.supplierSelections.unshift({
		supplierIdentity,
	...
	for (const line of booking.lines) {
		...
		if (!learning.glAccountSelections.some((decision) => decision.supplierAccountId === supplierAccountId && decision.descriptionKey === key && decision.glAccount === line.finalSelectedAccount)) learning.glAccountSelections.unshift({
			supplierAccountId,
			descriptionKey: key,
			glAccount: line.finalSelectedAccount,
			decidedAt
		});
		if (!learning.vatCodeSelections.some((decision) => decision.supplierAccountId === supplierAccountId && decision.descriptionKey === key && decision.vatCode === line.vatCode)) learning.vatCodeSelections.unshift({
			supplierAccountId,
			descriptionKey: key,
			vatCode: line.vatCode,
			decidedAt
		});
		if (line.costCentre && !learning.costCentreSelections.some((decision) => decision.supplierAccountId === supplierAccountId && decision.glAccount === line.finalSelectedAccount && decision.costCentre === line.costCentre)) learning.costCentreSelections.unshift({
			supplierAccountId,
			glAccount: line.finalSelectedAccount,
			costCentre: line.costCentre,
			decidedAt
		});
		if (line.costUnit && !learning.costUnitSelections.some((decision) => decision.supplierAccountId === supplierAccountId && decision.glAccount === line.finalSelectedAccount && decision.costUnit === line.costUnit)) learning.costUnitSelections.unshift({
			supplierAccountId,
			glAccount: line.finalSelectedAccount,
...
function supplierIdentityForInvoice(invoice) {
	return primarySupplierIdentity(invoice.extractedData);
}
//#endregion
//#region lib/services/exact-online-service.ts
function createExactAuthorizationUrl(state) {
	...
	const redirectUri = process.env.EXACT_ONLINE_REDIRECT_URI || "http://localhost:3000/api/exact/callback";
	return `${baseUrl}/api/oauth2/auth?${new URLSearchParams({
		client_id: clientId,
		redirect_uri: redirectUri,
		response_type: "code",
		force_login: "0",
...
function createMockExactConnection(userId) {
	const now = /* @__PURE__ */ new Date();
	return {
		id: createId("exact"),
		userId,
		...
		expiresAt: new Date(now.getTime() + 3300 * 1e3).toISOString(),
		scopes: [
			"financial",
			"purchase",
			"transaction"
...
async function refreshExactTokenIfNeeded(connection) {
	if (!connection) return null;
	if (new Date(connection.expiresAt).getTime() > Date.now() + 6e4) return connection;
	if (isRealExactMode() && !isMockExactConnection(connection)) return refreshRealExactConnection(connection);
	return {
		...connection,
		accessTokenCiphertext: "mock-refreshed-encrypted-access-token",
		expiresAt: new Date(Date.now() + 3300 * 1e3).toISOString(),
		updatedAt: (/* @__PURE__ */ new Date()).toISOString()
	};
}
async function bookInvoiceInExact(connection, invoice, masterData) {
	if (!connection || connection.status !== "connected") throw new Error("Exact Online is not connected.");
	if (isExactMasterDataStale(masterData)) throw new Error("Exact master data is missing or stale. Sync Exact data before booking.");
	const syncedMasterData = masterData;
	if (invoice.status !== "Ready to Book") throw new Error("Only invoices with Ready to Book status can be booked.");
	if (!invoice.purchaseJournal) throw new Error("Purchase Journal booking data is missing.");
	if (!invoice.purchaseJournal.attachmentPresent || !invoice.purchaseJournal.attachmentStorageKey) throw new Error("Original invoice attachment is required and must be sent to Exact Online.");
	if (!getStoredInvoiceFile(invoice.purchaseJournal.attachmentStorageKey)) throw new Error("Original invoice file is not available in storage.");
	if (!invoice.purchaseJournal.autoBookAllowed) throw new Error("Purchase Journal intelligence still requires user review.");
	const selectedSupplierId = invoice.purchaseJournal.supplierResolution.selectedAccountId;
	if (!selectedSupplierId || !syncedMasterData.suppliers.some((supplier) => supplier.id === selectedSupplierId)) throw new Error("Selected supplier is not available in Exact master data.");
	if (!syncedMasterData.journals.some((journal) => journal.code === invoice.purchaseJournal?.journal && journal.type === "purchase" && journal.isActive)) throw new Error("Selected purchase journal is not available in Exact.");
	for (const line of invoice.purchaseJournal.lines) {
		if (!syncedMasterData.glAccounts.some((account) => account.code === line.finalSelectedAccount && account.isActive)) throw new Error(`G/L account ${line.finalSelectedAccount} is not available in Exact.`);
		if (!syncedMasterData.vatCodes.some((vatCode) => vatCode.code === line.vatCode && vatCode.type === "purchase" && vatCode.isActive)) throw new Error(`Purchase VAT code ${line.vatCode} is not available in Exact.`);
		if (line.costCentre && !syncedMasterData.costCenters.some((costCenter) => costCenter.code === line.costCentre && costCenter.isActive)) throw new Error(`Cost center ${line.costCentre} is not available in Exact.`);
		if (line.costUnit && !syncedMasterData.costUnits.some((costUnit) => costUnit.code === line.costUnit && costUnit.isActive)) throw new Error(`Cost unit ${line.costUnit} is not available in Exact.`);
	}
	if (isRealExactMode() && !isMockExactConnection(connection)) return createRealExactPurchaseBooking(connection, invoice, syncedMasterData);
	if (/fail/i.test(invoice.extractedData.supplierName)) throw new Error("Mock Exact Online rejected the supplier ledger mapping.");
	return {
...
//#endregion
//#region lib/repository/invoice-store.ts
var rolePermissions = {
	Admin: [
		"view",
		"search_archive",
...
function exactMasterDataForUser(store, userId) {
...
function createSeedInvoice(overrides) {
	const createdAt = now();
	const ownerId = overrides.userId ?? "user_accountant";
	const ownerName = overrides.uploadedByName ?? (ownerId === "user_admin" ? "David Kwon" : "Tammy Park");
	const invoice = {
		id: createId("invoice"),
		userId: ownerId,
...
function createInitialStore() {
	...
	for (const invoice of store.invoices) store.auditEvents.push({
		id: createId("audit"),
...
function getStore() {
	if (!globalStore.__INTO_STORE || !Array.isArray(globalStore.__INTO_STORE.users) || !Array.isArray(globalStore.__INTO_STORE.exactConnections) || !Array.isArray(globalStore.__INTO_STORE.outlookConnections) || !Array.isArray(globalStore.__INTO_STORE.auditEvents)) globalStore.__INTO_STORE = createInitialStore();
...
function canUser(action, user = getCurrentUser()) {
...
function requirePermission(action) {
	const user = getCurrentUser();
	if (!canUser(action, user)) throw new Error(`${user.role} users are not allowed to ${action.replace(/_/g, " ")}.`);
	return user;
}
...
function listInvoices() {
	const store = getStore();
	for (const invoice of store.invoices) if (!invoice.deletedAt && !invoice.purchaseJournal && invoice.extractedData.invoiceNumber && invoice.status !== "Uploaded" && invoice.status !== "Reading") recomputeInvoiceInStore(store, invoice.id);
	return [...store.invoices].filter((invoice) => !invoice.deletedAt).sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
}
function getInvoice(invoiceId) {
	const store = getStore();
	const invoice = store.invoices.find((item) => item.id === invoiceId) ?? null;
	if (invoice && !invoice.purchaseJournal && invoice.extractedData.invoiceNumber && invoice.status !== "Uploaded" && invoice.status !== "Reading") return recomputeInvoiceInStore(store, invoice.id);
	return invoice;
}
function normalizedDuplicateText(value) {
	return (value ?? "").trim().toLowerCase().replace(/\s+/g, " ");
}
function invoiceReference(data) {
	return (data.referenceCode || data.invoiceNumber || "").trim();
}
function exactBookingReference(invoice) {
	return invoice.exactBookingId || invoice.bookingAttempts[0]?.exactBookingId;
}
function isBookedInExact(invoice) {
	return invoice.exactBookingStatus === "booked" || invoice.status === "Booked" || Boolean(exactBookingReference(invoice));
}
function isProcessedInvoice(invoice) {
	return Boolean(invoice.extractedData.invoiceNumber || invoice.extractedData.supplierName);
}
function duplicateCandidateFor(invoice, matchScore, matchReasons) {
	return {
		invoiceId: invoice.id,
		fileName: invoice.fileName,
...
function findDuplicateBeforeProcessing(input) {
	const checksumMatch = input.checksum ? getStore().invoices.find((invoice) => invoice.checksum === input.checksum) : void 0;
	const fallbackMatch = getStore().invoices.find((invoice) => invoice.fileName.trim().toLowerCase() === input.fileName.trim().toLowerCase() && invoice.fileSize === input.fileSize && (!input.checksum || !invoice.checksum || invoice.checksum === input.checksum));
	const duplicate = checksumMatch ?? fallbackMatch ?? null;
	if (!duplicate) return null;
	const candidate = duplicateCandidateFor(duplicate, checksumMatch ? 1 : .96, checksumMatch ? ["File checksum matches an existing INTO invoice."] : ["Filename and file size match an existing INTO invoice."]);
	const booked = isBookedInExact(duplicate);
	const outcome = booked ? "already_booked" : "processed_unbooked";
	const message = booked ? "This invoice has already been booked in Exact Online." : "This invoice was already processed but has not been booked in Exact Online yet. Do you want INTO to re-read it?";
	...
	if (booked) logDuplicateDecision({
		invoiceId: void 0,
...
function detectContentDuplicate(invoiceId) {
	...
	const candidates = getStore().invoices.filter((candidate) => candidate.id !== invoice.id && isProcessedInvoice(candidate)).map((candidate) => {
		const reasons = [];
		let score = 0;
		...
		if (reference && reference === normalizedDuplicateText(invoiceReference(candidateData))) {
			score += .35;
		...
		if (typeof total === "number" && typeof candidateData.grossAmount === "number" && Math.abs(total - candidateData.grossAmount) < .005) {
			score += .2;
	...
	}).filter((candidate) => candidate.matchScore >= .55).sort((a, b) => b.matchScore - a.matchScore);
...
function logDuplicateDecision(input) {
	const log = {
		id: createId("dup_log"),
		createdAt: now(),
		...input
...
function addAuditEvent(input) {
	const user = input.userId ? getStore().users.find((item) => item.id === input.userId) ?? getCurrentUser() : getCurrentUser();
	const event = {
		id: createId("audit"),
...
function listAuditEvents(invoiceId) {
	return [...invoiceId ? getStore().auditEvents.filter((event) => event.invoiceId === invoiceId) : getStore().auditEvents].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
}
function auditInvoiceFieldChanges(invoiceId, previous, next) {
	const changedFields = Object.keys(next).filter((field) => {
		if (field === "lineItems") return JSON.stringify(previous.lineItems) !== JSON.stringify(next.lineItems);
		return previous[field] !== next[field];
	});
	for (const field of changedFields) addAuditEvent({
		invoiceId,
...
function recomputeInvoiceInStore(store, invoiceId) {
	const invoice = store.invoices.find((item) => item.id === invoiceId);
	if (!invoice) return null;
	const baseValidationErrors = validateInvoiceData(invoice.id, invoice.extractedData, store.invoices.map((candidate) => ({
		id: candidate.id,
		supplierName: candidate.extractedData.supplierName,
		invoiceNumber: candidate.extractedData.invoiceNumber
	})));
	const purchaseJournal = generatePurchaseJournalBooking(invoice, store.invoices, store.learning, exactMasterDataForUser(store, store.currentUserId));
	const purchaseErrors = purchaseJournalValidationErrors(purchaseJournal);
	invoice.purchaseJournal = purchaseJournal;
	invoice.validationErrors = uniqueValidationErrors([...baseValidationErrors, ...purchaseErrors]);
	if (invoice.status !== "Booked" && invoice.status !== "Possible Duplicate") invoice.status = statusFromPurchaseJournal(baseValidationErrors, purchaseJournal);
	invoice.lastError = invoice.validationErrors.length ? invoice.validationErrors.map((item) => item.message).join(" ") : void 0;
	invoice.updatedAt = now();
	return invoice;
}
function recomputeInvoiceState(invoiceId) {
	return recomputeInvoiceInStore(getStore(), invoiceId);
}
function createUploadedInvoice(input) {
	const user = input.userId ? getStore().users.find((item) => item.id === input.userId) ?? getCurrentUser() : getCurrentUser();
	const timestamp = now();
	const invoice = {
		id: createId("invoice"),
	...
	getStore().invoices.unshift(invoice);
	addAuditEvent({
		invoiceId: invoice.id,
		userId: user.id,
		userName: userDisplayName(user),
		type: input.source === "outlook" ? "invoice_imported" : "invoice_uploaded",
		message: input.source === "outlook" ? `${userDisplayName(user)} imported ${input.fileName} from Outlook.` : `${userDisplayName(user)} uploaded ${input.fileName}.`,
		metadata: {
			source: input.source,
			fileName: input.fileName,
			checksum: input.checksum,
...
function updateInvoiceExtraction(invoiceId, extractedData) {
	const invoice = getInvoice(invoiceId);
	if (!invoice) return null;
	invoice.extractedData = extractedData;
	invoice.intelligenceApprovedAt = void 0;
	invoice.purchaseJournal = null;
	invoice.updatedAt = now();
	return invoice;
}
function markInvoiceReading(invoiceId) {
	const invoice = getInvoice(invoiceId);
	if (invoice) {
...
function markInvoiceFileError(invoiceId, message) {
	const invoice = getInvoice(invoiceId);
	if (!invoice) return null;
...
function markInvoicePossibleDuplicate(invoiceId, detection) {
	const invoice = getInvoice(invoiceId);
	if (!invoice) return null;
	invoice.status = "Possible Duplicate";
	invoice.duplicateDetection = detection;
	invoice.validationErrors = uniqueValidationErrors([...invoice.validationErrors, {
		id: createId("val"),
		field: "duplicate",
...
function pushExtractionHistory(invoice, reason, decision) {
	if (!isProcessedInvoice(invoice)) return;
...
function replaceInvoiceExtractionFromReread(invoiceId, extractedData, decision = "re_read") {
	const invoice = getInvoice(invoiceId);
	if (!invoice) return null;
	pushExtractionHistory(invoice, "duplicate_re_read", decision);
	invoice.extractedData = extractedData;
	invoice.duplicateResolutionDecision = decision;
	invoice.duplicateDetection = void 0;
	invoice.intelligenceApprovedAt = void 0;
	invoice.purchaseJournal = null;
	invoice.lastError = void 0;
	invoice.updatedAt = now();
	addAuditEvent({
		invoiceId,
		type: "invoice_reread",
...
function resolveDuplicateDecision(input) {
	...
	if (invoice) {
		...
		if (input.decision === "continue_anyway" && invoice.status === "Possible Duplicate") {
			invoice.duplicateDetection = void 0;
			invoice.validationErrors = invoice.validationErrors.filter((error) => error.field !== "duplicate");
			invoice.lastError = void 0;
...
function addBookingAttempt(invoiceId, attempt) {
	const invoice = getInvoice(invoiceId);
	if (!invoice) return null;
...
function markInvoiceBooked(invoiceId, exactBookingId) {
	const invoice = getInvoice(invoiceId);
	if (!invoice) return null;
	...
	addAuditEvent({
		...
		metadata: { exactBookingId }
	});
	return invoice;
}
function markInvoiceBookingFailed(invoiceId, message) {
	const invoice = getInvoice(invoiceId);
	if (!invoice) return null;
...
function approveInvoiceIntelligence(invoiceId) {
	const invoice = getInvoice(invoiceId);
	if (!invoice) return null;
...
function selectInvoiceSupplier(invoiceId, accountId) {
	const invoice = getInvoice(invoiceId);
	const store = getStore();
	const account = exactMasterDataForUser(store, store.currentUserId)?.suppliers.find((supplier) => supplier.id === accountId) ?? null;
	if (!invoice || !account) return null;
	const supplierIdentity = supplierIdentityForInvoice(invoice);
	if (!store.learning.supplierSelections.some((decision) => decision.supplierIdentity === supplierIdentity && decision.accountId === account.id)) store.learning.supplierSelections.unshift({
		supplierIdentity,
...
function setExactConnection(connection) {
	const store = getStore();
	store.exactConnections = [connection, ...store.exactConnections.filter((item) => item.userId !== connection.userId)];
	addAuditEvent({
...
function getExactConnection(userId = getCurrentUser().id) {
	return getStore().exactConnections.find((connection) => connection.userId === userId) ?? null;
}
function publicExactConnection(connection = getExactConnection()) {
	if (!connection) return null;
	return {
		id: connection.id,
		userId: connection.userId,
...
function getExactMasterData(userId = getCurrentUser().id) {
	return exactMasterDataForUser(getStore(), userId);
}
function isCachedExactMasterDataStale(userId = getCurrentUser().id) {
	return isExactMasterDataStale(getExactMasterData(userId));
}
async function syncExactDataNow(userId = getCurrentUser().id) {
	const store = getStore();
	const connection = await refreshExactConnectionForUser(userId);
	if (connection) store.exactConnections = [connection, ...store.exactConnections.filter((item) => item.userId !== userId)];
	...
	for (const invoice of store.invoices) if (invoice.status !== "Uploaded" && invoice.status !== "Reading") recomputeInvoiceInStore(store, invoice.id);
	addAuditEvent({
		userId,
		type: "sync_operation",
...
async function refreshExactConnectionForUser(userId = getCurrentUser().id) {
	...
	try {
		...
		if (refreshed && refreshed.updatedAt !== connection.updatedAt) {
			getStore().exactConnections = [refreshed, ...getStore().exactConnections.filter((item) => item.userId !== userId)];
			addAuditEvent({
				userId,
		...
		markExactConnectionNeedsReconnect(userId, error instanceof Error ? error.message : "Exact Online token refresh failed.");
		throw error;
	}
}
function markExactConnectionNeedsReconnect(userId = getCurrentUser().id, reason = "Exact Online access must be re-authorized.") {
	const store = getStore();
	const connection = getExactConnection(userId);
	if (!connection) return null;
	...
	store.exactConnections = [updatedConnection, ...store.exactConnections.filter((item) => item.userId !== userId)];
	addAuditEvent({
		userId,
...
function disconnectExactConnection(userId = getCurrentUser().id) {
	const store = getStore();
	const connection = getExactConnection(userId);
	store.exactConnections = store.exactConnections.filter((item) => item.userId !== userId);
	store.exactMasterDataCaches = store.exactMasterDataCaches.filter((item) => item.userId !== userId);
	if (connection) addAuditEvent({
		userId,
...
function setOutlookConnection(connection) {
	const store = getStore();
	store.outlookConnections = [connection, ...store.outlookConnections.filter((item) => item.userId !== connection.userId)];
	addAuditEvent({
		userId: connection.userId,
		type: "connection_connected",
...
function getOutlookConnection(userId = getCurrentUser().id) {
	return getStore().outlookConnections.find((connection) => connection.userId === userId) ?? null;
}
function publicOutlookConnection(connection = getOutlookConnection()) {
	if (!connection) return null;
	return {
		id: connection.id,
		userId: connection.userId,
...
function setOutlookConnectionNeedsReconnect(userId = getCurrentUser().id, reason = "Outlook access must be re-authorized.") {
	const store = getStore();
	...
	store.outlookConnections = [updatedConnection, ...store.outlookConnections.filter((item) => item.userId !== userId)];
	addAuditEvent({
		userId,
		type: "token_refresh_failure",
...
function disconnectOutlookConnection(userId = getCurrentUser().id) {
	const store = getStore();
	const connection = getOutlookConnection(userId);
	store.outlookConnections = store.outlookConnections.filter((item) => item.userId !== userId);
	if (connection) addAuditEvent({
		userId,
		type: "connection_disconnected",
...
function invoiceKeywordText(invoice) {
	const lineDescriptions = invoice.purchaseJournal?.lines.map((line) => line.description).join(" ") ?? "";
	return [
		invoice.fileName,
...
function invoiceValidationStatus(invoice) {
	if (invoice.validationErrors.some((error) => error.severity === "error")) return "error";
	if (invoice.validationErrors.length) return "warning";
	return "valid";
}
function invoiceDuplicateStatus(invoice) {
	return invoice.duplicateDetection?.outcome ?? "none";
}
function invoiceSortValue(invoice, sortBy) {
	switch (sortBy) {
		case "invoiceDate": return invoice.extractedData.invoiceDate;
		case "supplier": return invoice.extractedData.supplierName;
		case "amount": return invoice.extractedData.grossAmount ?? 0;
		case "status": return invoice.status;
		case "uploader": return invoice.uploadedByName;
		default: return invoice.createdAt;
	}
}
function searchInvoiceArchive(filters) {
	...
	const filtered = getStore().invoices.filter((invoice) => !invoice.deletedAt).filter((invoice) => keyword ? invoiceKeywordText(invoice).includes(keyword) : true).filter((invoice) => dateWithin(invoice.extractedData.invoiceDate || void 0, filters.invoiceDateFrom, filters.invoiceDateTo)).filter((invoice) => dateWithin(invoice.createdAt, filters.uploadedAtFrom, filters.uploadedAtTo)).filter((invoice) => supplier ? includesText(invoice.extractedData.supplierName, supplier) : true).filter((invoice) => {
		const amount = invoice.extractedData.grossAmount;
		if (typeof filters.amountMin === "number" && (amount ?? -Infinity) < filters.amountMin) return false;
		if (typeof filters.amountMax === "number" && (amount ?? Infinity) > filters.amountMax) return false;
		return true;
	}).filter((invoice) => filters.currency ? invoice.extractedData.currency === filters.currency : true).filter((invoice) => invoiceNumber ? includesText(invoice.extractedData.invoiceNumber, invoiceNumber) || includesText(invoice.extractedData.referenceCode, invoiceNumber) : true).filter((invoice) => filters.bookingStatus ? invoice.status === filters.bookingStatus : true).filter((invoice) => filters.validationStatus ? invoiceValidationStatus(invoice) === filters.validationStatus : true).filter((invoice) => filters.source ? invoice.source === filters.source : true).filter((invoice) => filters.uploadedByUserId ? invoice.uploadedByUserId === filters.uploadedByUserId : true).filter((invoice) => exactBookingReference ? includesText(invoice.exactBookingId, exactBookingReference) : true).filter((invoice) => filters.journal ? invoice.purchaseJournal?.journal === filters.journal : true).filter((invoice) => filters.glAccount ? invoice.purchaseJournal?.lines.some((line) => line.glAccount === filters.glAccount) : true).filter((invoice) => filters.vatCode ? invoice.purchaseJournal?.lines.some((line) => line.vatCode === filters.vatCode) : true).filter((invoice) => filters.costCenter ? invoice.purchaseJournal?.lines.some((line) => line.costCentre === filters.costCenter) : true).filter((invoice) => filters.costUnit ? invoice.purchaseJournal?.lines.some((line) => line.costUnit === filters.costUnit) : true).filter((invoice) => filters.country ? invoice.extractedData.supplierCountry === filters.country : true).filter((invoice) => filters.duplicateStatus ? invoiceDuplicateStatus(invoice) === filters.duplicateStatus : true).sort((a, b) => {
...
//#endregion
//#region app/api/invoices/[invoiceId]/audit/route.ts
var route_exports$19 = /* @__PURE__ */ __exportAll({ GET: () => GET$11 });
async function invoiceIdFromContext$5(context) {
	return (await context.params).invoiceId;
}
async function GET$11(_request, context) {
	try {
		requirePermission("view");
		const invoiceId = await invoiceIdFromContext$5(context);
		const invoice = getInvoice(invoiceId);
		...
		const message = error instanceof Error ? error.message : "Audit trail failed.";
...
//#endregion
//#region app/api/invoices/[invoiceId]/book/route.ts
var route_exports$18 = /* @__PURE__ */ __exportAll({ POST: () => POST$11 });
async function invoiceIdFromContext$4(context) {
	return (await context.params).invoiceId;
}
async function POST$11(_request, context) {
	const invoiceId = await invoiceIdFromContext$4(context);
	const invoice = getInvoice(invoiceId);
	if (!invoice) return Response.json({ error: "Invoice not found." }, { status: 404 });
	try {
	...
	try {
		...
		if (isCachedExactMasterDataStale()) masterData = await syncExactDataNow();
		const result = await bookInvoiceInExact(connection, recomputeInvoiceState(invoice.id) ?? invoice, masterData);
		...
		logger.error("invoice.booking_failed", {
			invoiceId,
			message
		});
		return Response.json({
...
//#endregion
//#region lib/services/invoice-extraction-service.ts
var supplierProfiles = [
	{
		name: "Noordzee Office Supplies",
		vat: "NL812345678B01",
		iban: "NL91ABNA0417164300",
...
function invoiceNumberFromFile(fileName) {
	return `INV-${fileName.replace(/\.[^.]+$/, "").replace(/[^a-z0-9]+/gi, "-").slice(0, 14).toUpperCase() || "UPLOAD"}`;
}
...
function profileForFile(fileName, seed) {
	if (/google|workspace|reverse|eu-acquisition|saas|subscription/i.test(fileName)) return namedProfiles.google;
	if (/booking|hotel|restaurant/i.test(fileName)) return namedProfiles.booking;
	if (/klm|flight|airline|eurostar/i.test(fileName)) return namedProfiles.klm;
	if (/insurance|policy/i.test(fileName)) return namedProfiles.insurance;
	if (/inbody|intercompany|internal/i.test(fileName)) return namedProfiles.inbody;
	if (/ambiguous|acme|multiple-supplier/i.test(fileName)) return namedProfiles.ambiguous;
	if (/non-eu|outside-eu|us-cloud|(^|[^a-z])us([^a-z]|$)/i.test(fileName)) return namedProfiles.us;
	return supplierProfiles[seed % supplierProfiles.length];
}
function vatRateForFile(fileName, profile) {
	if (/reverse|eu-acquisition|non-eu|outside-eu|flight|klm|hotel|booking|insurance|restaurant|zero-vat/i.test(fileName)) return 0;
	if (/nine|9-vat|restaurant-vat/i.test(fileName)) return .09;
	if (profile.country !== "NL") return 0;
	return .21;
}
function servicePeriodFor(fileName, invoiceDate) {
	if (/subscription|saas|google|insurance|maintenance/i.test(fileName)) return {
		start: "2026-01-15",
		end: "2026-06-30"
	};
...
async function extractInvoiceData(file) {
	...
	const invoiceDate = /closed-period|january|february/i.test(file.name) ? new Date(Date.UTC(2026, 0, seed % 24 + 1)) : new Date(Date.UTC(2026, 5 + seed % 2, seed % 24 + 1));
	...
	const reverseCharge = /reverse|eu-acquisition|intra-community/i.test(file.name) || profile.name === "Google Ireland Limited";
	const servicePeriod = servicePeriodFor(file.name, invoiceDate);
	const beneficiary = /david|kwon|flight|hotel|booking/i.test(file.name) ? "David Kwon" : "";
	return {
		...
		referenceCode: /reference/i.test(file.name) ? `REF-${String(seed).slice(0, 5)}` : "",
		...
		netAmount,
		vatAmount,
		grossAmount: mismatchByName ? grossAmount + .01 : grossAmount,
		iban: profile.iban,
		...
		confidence: invalidByName ? .72 : .94,
		rawText: [
...
//#endregion
//#region app/api/invoices/[invoiceId]/duplicate-resolution/route.ts
var route_exports$17 = /* @__PURE__ */ __exportAll({ POST: () => POST$10 });
async function invoiceIdFromContext$3(context) {
	return (await context.params).invoiceId;
}
async function POST$10(request, context) {
	const invoiceId = await invoiceIdFromContext$3(context);
	const invoice = getInvoice(invoiceId);
	if (!invoice) return Response.json({ error: "Invoice not found." }, { status: 404 });
	try {
	...
	try {
		...
		const detectionOutcome = payload.detectionOutcome ?? invoice.duplicateDetection?.outcome ?? "processed_unbooked";
		const message = payload.message ?? invoice.duplicateDetection?.message ?? "Duplicate invoice decision recorded.";
		if (decision === "re_read") {
			const storedFile = getStoredInvoiceFile(invoice.storageKey);
			if (!storedFile) return Response.json({ error: "Original invoice file is not available for re-reading." }, { status: 409 });
			const extractedData = await extractInvoiceData({
				name: storedFile.fileName,
				type: storedFile.fileType,
				size: storedFile.fileSize
			});
			const updatedInvoice = replaceInvoiceExtractionFromReread(invoice.id, extractedData, "re_read");
		...
		const message = error instanceof Error ? error.message : "Duplicate decision failed.";
		logger.error("invoice.duplicate_decision_failed", {
			invoiceId,
			message
		});
...
//#endregion
//#region app/api/invoices/[invoiceId]/file/route.ts
var route_exports$16 = /* @__PURE__ */ __exportAll({ GET: () => GET$10 });
async function invoiceIdFromContext$2(context) {
	return (await context.params).invoiceId;
}
...
async function GET$10(request, context) {
	const invoice = getInvoice(await invoiceIdFromContext$2(context));
	if (!invoice) return Response.json({ error: "Invoice not found." }, { status: 404 });
	const storedFile = getStoredInvoiceFile(invoice.storageKey);
	if (!storedFile) return Response.json({ error: "Original invoice source document is not available." }, { status: 404 });
	const download = new URL(request.url).searchParams.get("download") === "1";
	const body = storedFile.bytes.buffer.slice(storedFile.bytes.byteOffset, storedFile.bytes.byteOffset + storedFile.bytes.byteLength);
	return new Response(body, { headers: {
...
//#endregion
//#region app/api/invoices/[invoiceId]/intelligence/route.ts
var route_exports$15 = /* @__PURE__ */ __exportAll({ POST: () => POST$9 });
async function invoiceIdFromContext$1(context) {
	return (await context.params).invoiceId;
}
async function POST$9(request, context) {
	try {
		requirePermission("approve");
		const invoiceId = await invoiceIdFromContext$1(context);
		if (!getInvoice(invoiceId)) return Response.json({ error: "Invoice not found." }, { status: 404 });
		const payload = await request.json();
		const updatedInvoice = payload.action === "selectSupplier" ? selectInvoiceSupplier(invoiceId, payload.accountId ?? "") : approveInvoiceIntelligence(invoiceId);
		if (!updatedInvoice) return Response.json({ error: "Unable to apply purchase journal decision." }, { status: 400 });
		...
		const message = error instanceof Error ? error.message : "Unexpected intelligence action error";
		logger.error("invoice.intelligence_action_failed", { message });
		return Response.json({ error: message }, { status: message.includes("not allowed") ? 403 : 500 });
	}
}
//#endregion
//#region components/into-workbench.tsx
var IntoWorkbench = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'IntoWorkbench' is called on server");
}, "ccf4248b4216", "IntoWorkbench");
//#endregion
//#region app/page.tsx
var page_exports = /* @__PURE__ */ __exportAll({ default: () => Home });
function Home() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(IntoWorkbench, {});
}
//#endregion
//#region app/api/exact/callback/route.ts
...
async function GET$9(request) {
	...
	if (isRealExactMode()) try {
		if (!code || !state) throw new Error("Exact OAuth callback is missing code or state.");
		const verifiedState = await verifyExactOAuthState(state);
		const connection = setExactConnection(await exchangeExactAuthorizationCode(verifiedState.userId, code));
		...
		const message = callbackError instanceof Error ? callbackError.message : "Exact OAuth callback failed.";
		logger.error("exact.oauth_callback_failed", { message });
		return Response.redirect(new URL("/?exact=error", request.url));
	}
	const hasCode = Boolean(code);
	const connection = setExactConnection(createMockExactConnection(getDemoUserId()));
	logger.info("exact.oauth_callback", {
		connectionId: connection.id,
		hasCode
	});
...
//#endregion
//#region app/api/exact/connect/route.ts
...
async function GET$8() {
	try {
		requirePermission("connect_exact");
		if (isRealExactMode()) {
			const authorization = await createRealExactAuthorizationUrl(getDemoUserId());
			return Response.json({
				...authorization,
				mode: "real",
		...
		const message = error instanceof Error ? error.message : "Exact Online connection failed.";
		logger.error("exact.connect_metadata_failed", { message });
		return Response.json({ error: message }, { status: message.includes("not allowed") ? 403 : 409 });
	}
}
...
//#endregion
//#region app/api/exact/disconnect/route.ts
var route_exports$12 = /* @__PURE__ */ __exportAll({ POST: () => POST$7 });
async function POST$7() {
	try {
		requirePermission("connect_exact");
		const disconnected = disconnectExactConnection();
		logger.info("exact.disconnected", { connectionId: disconnected?.id });
		return Response.json({
		...
		const message = error instanceof Error ? error.message : "Exact disconnect failed.";
		return Response.json({ error: message }, { status: message.includes("not allowed") ? 403 : 409 });
	}
}
//#endregion
//#region app/api/exact/status/route.ts
...
//#endregion
//#region app/api/exact/sync/route.ts
var route_exports$10 = /* @__PURE__ */ __exportAll({ POST: () => POST$6 });
...
//#endregion
//#region app/api/invoices/route.ts
...
async function POST$5(request) {
	try {
		...
		const checksums = formData.getAll("checksums").map((value) => typeof value === "string" ? value : "");
		if (files.length === 0) return Response.json({ error: "Upload at least one invoice file." }, { status: 400 });
		const processed = [];
		const rejected = [];
		const duplicates = [];
		for (const [index, file] of files.entries()) {
			const checksum = checksums[index] || void 0;
			...
			if (!isSupportedInvoiceFile(file.name)) {
				processed.push(markInvoiceFileError(invoice.id, "Unsupported file type. Upload PDF, JPG, PNG, XML, or UBL invoices."));
				continue;
			}
			...
			const finalInvoice = detectContentDuplicate(invoice.id)?.outcome === "possible_duplicate" ? recomputeInvoiceState(invoice.id) : validatedInvoice;
...
//#endregion
//#region app/api/invoices/archive/route.ts
var route_exports$8 = /* @__PURE__ */ __exportAll({ GET: () => GET$5 });
...
async function GET$5(request) {
	try {
		...
		const filters = {
			keyword: params.get("keyword") ?? void 0,
			invoiceDateFrom: params.get("invoiceDateFrom") ?? void 0,
			invoiceDateTo: params.get("invoiceDateTo") ?? void 0,
			uploadedAtFrom: params.get("uploadedAtFrom") ?? void 0,
			uploadedAtTo: params.get("uploadedAtTo") ?? void 0,
			supplier: params.get("supplier") ?? void 0,
			amountMin: numberParam(params, "amountMin"),
			amountMax: numberParam(params, "amountMax"),
			currency: params.get("currency") ?? void 0,
			invoiceNumber: params.get("invoiceNumber") ?? void 0,
			bookingStatus: params.get("bookingStatus") ?? void 0,
			validationStatus: params.get("validationStatus") ?? void 0,
			source: params.get("source") ?? void 0,
			uploadedByUserId: params.get("uploadedByUserId") ?? void 0,
			exactBookingReference: params.get("exactBookingReference") ?? void 0,
			journal: params.get("journal") ?? void 0,
			glAccount: params.get("glAccount") ?? void 0,
			vatCode: params.get("vatCode") ?? void 0,
			costCenter: params.get("costCenter") ?? void 0,
			costUnit: params.get("costUnit") ?? void 0,
			country: params.get("country") ?? void 0,
			duplicateStatus: params.get("duplicateStatus") ?? void 0,
			sortBy: params.get("sortBy") ?? void 0,
...
//#endregion
//#region app/api/invoices/book-ready/route.ts
var route_exports$7 = /* @__PURE__ */ __exportAll({ POST: () => POST$4 });
async function POST$4() {
	...
	const readyInvoices = listInvoices().filter((invoice) => invoice.status === "Ready to Book");
	for (const invoice of readyInvoices) try {
		...
		logger.error("invoice.bulk_booking_failed", {
			invoiceId: invoice.id,
			message
		});
	}
...
//#endregion
//#region lib/services/outlook-service.ts
...
function microsoftConfig() {
	...
	const redirectUri = process.env.MICROSOFT_REDIRECT_URI || `${process.env.APP_URL || "http://localhost:3000"}/api/outlook/callback`;
	if (isRealOutlookMode() && (!clientId || !clientSecret || !redirectUri)) throw new Error("MICROSOFT_CLIENT_ID, MICROSOFT_CLIENT_SECRET, and MICROSOFT_REDIRECT_URI are required for real Outlook OAuth.");
	return {
		tenantId,
		clientId,
		clientSecret,
...
function outlookIntegrationMode() {
	const explicit = process.env.MICROSOFT_OUTLOOK_MODE?.toLowerCase();
	if (explicit === "real" || explicit === "mock") return explicit;
	return process.env.MICROSOFT_CLIENT_ID && process.env.MICROSOFT_CLIENT_SECRET ? "real" : "mock";
}
function isRealOutlookMode() {
	return outlookIntegrationMode() === "real";
}
function isMockOutlookConnection(connection) {
	return !connection || connection.accessTokenCiphertext.startsWith("mock-") || connection.refreshTokenCiphertext.startsWith("mock-");
}
async function createMicrosoftOAuthState(userId) {
	const payload = encodeExactStatePayload({
		userId,
...
async function verifyMicrosoftOAuthState(state) {
	const [payload, signature] = state.split(".");
	if (!payload || !signature) throw new Error("Outlook OAuth state is missing or invalid.");
	if (signature !== await signExactState(payload)) throw new Error("Outlook OAuth state signature is invalid.");
	const decoded = decodeExactStatePayload(payload);
	if (Date.now() - decoded.issuedAt > 900 * 1e3) throw new Error("Outlook OAuth state has expired. Start the connection again.");
	return decoded;
}
function createOutlookAuthorizationUrl(state) {
	const tenantId = microsoftTenantId();
...
async function createRealOutlookAuthorizationUrl(userId) {
...
async function connectionFromMicrosoftToken(userId, token) {
	const now = /* @__PURE__ */ new Date();
	const expiresInSeconds = token.expires_in ?? 3600;
	if (!token.refresh_token) throw new Error("Outlook OAuth response did not include a refresh token.");
	const profile = await fetchMicrosoftProfile(token.access_token);
	const mailboxAddress = profile.mail || profile.userPrincipalName || profile.displayName || "unknown";
	return {
		id: createId("outlook"),
		userId,
		mailboxAddress,
...
async function exchangeOutlookAuthorizationCode(userId, code) {
	return connectionFromMicrosoftToken(userId, await requestMicrosoftToken({
		grant_type: "authorization_code",
		code
	}));
}
async function refreshRealOutlookConnection(connection) {
	const token = await requestMicrosoftToken({
		grant_type: "refresh_token",
		refresh_token: await decryptOAuthSecret(connection.refreshTokenCiphertext)
	});
	const now = /* @__PURE__ */ new Date();
	const expiresInSeconds = token.expires_in ?? 3600;
	return {
		...connection,
		accessTokenCiphertext: await encryptOAuthSecret(token.access_token),
		refreshTokenCiphertext: token.refresh_token ? await encryptOAuthSecret(token.refresh_token) : connection.refreshTokenCiphertext,
		expiresAt: new Date(now.getTime() + expiresInSeconds * 1e3).toISOString(),
		updatedAt: now.toISOString()
	};
}
async function refreshOutlookTokenIfNeeded(connection) {
	if (!connection) return null;
	if (new Date(connection.expiresAt).getTime() > Date.now() + 6e4) return connection;
	if (isRealOutlookMode() && !isMockOutlookConnection(connection)) return refreshRealOutlookConnection(connection);
	return {
		...connection,
...
function createMockOutlookConnection(userId) {
	const now = /* @__PURE__ */ new Date();
	return {
		id: createId("outlook"),
		userId,
...
function mockDetectedInvoiceEmails() {
	return [{
		...
		attachmentSize: 94120
	}];
}
//#endregion
//#region app/api/outlook/callback/route.ts
var route_exports$6 = /* @__PURE__ */ __exportAll({ GET: () => GET$4 });
async function GET$4(request) {
	...
	try {
		if (!code || !state) throw new Error("Outlook OAuth callback is missing code or state.");
		const connection = setOutlookConnection(await exchangeOutlookAuthorizationCode((await verifyMicrosoftOAuthState(state)).userId, code));
		...
		const message = callbackError instanceof Error ? callbackError.message : "Outlook OAuth callback failed.";
		logger.error("outlook.oauth_callback_failed", { message });
		return Response.redirect(new URL("/?outlook=error", request.url));
	}
}
//#endregion
//#region app/api/outlook/connect/route.ts
...
async function GET$3() {
	...
	if (isRealOutlookMode()) {
		const authorization = await createRealOutlookAuthorizationUrl(getDemoUserId());
		return Response.json({
			...authorization,
			mode: "real",
			requiresRedirect: true
...
async function POST$3() {
	...
	const connection = setOutlookConnection(createMockOutlookConnection(getDemoUserId()));
...
//#endregion
//#region app/api/outlook/disconnect/route.ts
var route_exports$4 = /* @__PURE__ */ __exportAll({ POST: () => POST$2 });
async function POST$2() {
	try {
		requirePermission("connect_outlook");
		const disconnected = disconnectOutlookConnection();
		logger.info("outlook.disconnected", { connectionId: disconnected?.id });
		return Response.json({ connection: publicOutlookConnection() });
	} catch (error) {
		const message = error instanceof Error ? error.message : "Outlook disconnect failed.";
		return Response.json({ error: message }, { status: message.includes("not allowed") ? 403 : 409 });
	}
}
//#endregion
//#region app/api/outlook/ingest/route.ts
...
async function POST$1() {
	...
	if (!connection || connection.status !== "connected") return Response.json({ error: "Connect Outlook before ingestion." }, { status: 409 });
	try {
		...
		const message = error instanceof Error ? error.message : "Outlook token refresh failed.";
		setOutlookConnectionNeedsReconnect(connection.userId, message);
		return Response.json({ error: "Outlook access was revoked or expired. Reconnect Outlook." }, { status: 409 });
	}
	...
	for (const email of detectedEmails) {
		...
		const log = addOutlookLog({
			...
			subject: email.subject,
			sender: email.sender,
			category: finalInvoice?.status === "Ready to Book" ? "Processed" : "Needs Review",
			detectedAttachmentCount: 1,
...
//#endregion
//#region app/api/outlook/status/route.ts
...
async function POST(request) {
	const context = switchCurrentUser((await request.json()).userId ?? "");
	if (!context) return Response.json({ error: "User not found or disabled." }, { status: 404 });
	return Response.json({
		users: listUsers(),
...
//#endregion
//#region app/api/invoices/[invoiceId]/route.ts
var route_exports = /* @__PURE__ */ __exportAll({
	GET: () => GET,
	PATCH: () => PATCH
});
async function invoiceIdFromContext(context) {
	return (await context.params).invoiceId;
}
...
var routes = [
	{
		...
		ids: {
			"route": "route:/api/invoices/:invoiceId/audit",
			"page": null,
			"routeHandler": "route-handler:/api/invoices/:invoiceId/audit",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/invoices/:invoiceId/audit",
		patternParts: [
			"api",
			"invoices",
			":invoiceId",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/invoices/:invoiceId/book",
			"page": null,
			"routeHandler": "route-handler:/api/invoices/:invoiceId/book",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/invoices/:invoiceId/book",
		patternParts: [
			"api",
			"invoices",
			":invoiceId",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/invoices/:invoiceId/duplicate-resolution",
			"page": null,
			"routeHandler": "route-handler:/api/invoices/:invoiceId/duplicate-resolution",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/invoices/:invoiceId/duplicate-resolution",
		patternParts: [
			"api",
			"invoices",
			":invoiceId",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/invoices/:invoiceId/file",
			"page": null,
			"routeHandler": "route-handler:/api/invoices/:invoiceId/file",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/invoices/:invoiceId/file",
		patternParts: [
			"api",
			"invoices",
			":invoiceId",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/invoices/:invoiceId/intelligence",
			"page": null,
			"routeHandler": "route-handler:/api/invoices/:invoiceId/intelligence",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/invoices/:invoiceId/intelligence",
		patternParts: [
			"api",
			"invoices",
			":invoiceId",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/exact/callback",
			"page": null,
			"routeHandler": "route-handler:/api/exact/callback",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/exact/callback",
		patternParts: [
			"api",
			"exact",
			"callback"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/exact/connect",
			"page": null,
			"routeHandler": "route-handler:/api/exact/connect",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/exact/connect",
		patternParts: [
			"api",
			"exact",
			"connect"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/exact/disconnect",
			"page": null,
			"routeHandler": "route-handler:/api/exact/disconnect",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/exact/disconnect",
		patternParts: [
			"api",
			"exact",
			"disconnect"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/exact/status",
			"page": null,
			"routeHandler": "route-handler:/api/exact/status",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/exact/status",
		patternParts: [
			"api",
			"exact",
			"status"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/exact/sync",
			"page": null,
			"routeHandler": "route-handler:/api/exact/sync",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/exact/sync",
		patternParts: [
			"api",
			"exact",
			"sync"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/invoices",
			"page": null,
			"routeHandler": "route-handler:/api/invoices",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/invoices/archive",
			"page": null,
			"routeHandler": "route-handler:/api/invoices/archive",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/invoices/archive",
		patternParts: [
			"api",
			"invoices",
			"archive"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/invoices/book-ready",
			"page": null,
			"routeHandler": "route-handler:/api/invoices/book-ready",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/invoices/book-ready",
		patternParts: [
			"api",
			"invoices",
			"book-ready"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/outlook/callback",
			"page": null,
			"routeHandler": "route-handler:/api/outlook/callback",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/outlook/callback",
		patternParts: [
			"api",
			"outlook",
			"callback"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/outlook/connect",
			"page": null,
			"routeHandler": "route-handler:/api/outlook/connect",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/outlook/connect",
		patternParts: [
			"api",
			"outlook",
			"connect"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/outlook/disconnect",
			"page": null,
			"routeHandler": "route-handler:/api/outlook/disconnect",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/outlook/disconnect",
		patternParts: [
			"api",
			"outlook",
			"disconnect"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/outlook/ingest",
			"page": null,
			"routeHandler": "route-handler:/api/outlook/ingest",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/outlook/ingest",
		patternParts: [
			"api",
			"outlook",
			"ingest"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/outlook/status",
			"page": null,
			"routeHandler": "route-handler:/api/outlook/status",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/outlook/status",
		patternParts: [
			"api",
			"outlook",
			"status"
	...
	},
	{
		...
		ids: {
			...
			"page": null,
			"routeHandler": "route-handler:/api/users",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/invoices/:invoiceId",
			"page": null,
			"routeHandler": "route-handler:/api/invoices/:invoiceId",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/invoices/:invoiceId",
		patternParts: [
			"api",
			"invoices",
			":invoiceId"
