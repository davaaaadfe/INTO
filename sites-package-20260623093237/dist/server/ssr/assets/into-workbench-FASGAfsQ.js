...
function IntoWorkbench() {
	...
	(0, import_react.useEffect)(() => {
		...
		if (!exactStatus && !outlookStatus) return;
		const timeoutId = window.setTimeout(() => {
			if (exactStatus) setMessage(exactStatus === "connected" ? "Exact Online is connected. INTO synced Exact master data." : "Exact Online connection failed. Check your Exact app credentials and redirect URI.");
			else if (outlookStatus) setMessage(outlookStatus === "connected" ? "Outlook is connected with Microsoft OAuth." : "Outlook connection failed. Check your Microsoft app credentials and redirect URI.");
	...
	async function connectOutlook() {
		...
		try {
			...
			const data = await response.json();
			if (!response.ok) throw new Error(data.error ?? "Outlook connection failed.");
			if (data.requiresRedirect && data.authorizationUrl) {
			...
			setMessage(data.mode === "real" ? "Outlook is connected with Microsoft OAuth." : "Outlook mock connection is active.");
	...
	async function disconnectExact() {
		...
		try {
			...
			const data = await response.json();
			if (!response.ok) throw new Error(data.error ?? "Exact disconnect failed.");
			setState((current) => ({
				...current,
				invoices: data.invoices ?? current.invoices,
				exactConnection: data.connection,
	...
	async function disconnectOutlook() {
		...
		try {
			...
			const data = await response.json();
			if (!response.ok) throw new Error(data.error ?? "Outlook disconnect failed.");
			setState((current) => ({
				...current,
				outlookConnection: data.connection
			}));
								...
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ActionButton, {
											...
											disabled: !state.exactConnection || busy === "exact-disconnect" || !hasPermission("connect_exact"),
											disabledReason: !state.exactConnection ? "Exact Online is not connected." : "Your role cannot disconnect Exact Online.",
											children: "Disconnect Exact"
										})
									]
								})
							]
								...
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ActionButton, {
											...
											feedback: buttonFeedbackFor("outlook-disconnect", "outlook-disconnect"),
											disabled: !state.outlookConnection || busy === "outlook-disconnect" || !hasPermission("connect_outlook"),
											disabledReason: !state.outlookConnection ? "Outlook is not connected." : "Your role cannot disconnect Outlook.",
											children: "Disconnect Outlook"
										})
									]
								})
							]
