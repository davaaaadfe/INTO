...
export function IntoWorkbench() {
  ...
  async function refreshAll() {
    const [invoiceResponse, exactResponse, outlookResponse, userResponse] =
      await Promise.all([
      fetch("/api/invoices"),
  ...
  function bookDisabledReason(invoice: UploadedInvoice) {
    if (!hasPermission("book")) {
      return "Your role does not allow booking invoices.";
    }

    if (!state.exactConnection) {
      return "Connect Exact Online before booking.";
    }
      ...
      <div className="mx-auto flex max-w-[1560px] flex-col gap-5 px-5 py-5 lg:px-8">
        ...
        <section className="grid gap-4 xl:grid-cols-[1.45fr_0.55fr]">
                ...
                <ActionButton
                  ...
                  disabledReason={
                    !hasPermission("upload")
                      ? "Your role does not allow invoice uploads."
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-1">
            <div className="rounded-lg border border-stone-300 bg-white p-4">
              <h2 className="text-lg font-semibold">Exact Online</h2>
              <p className="mt-1 text-sm text-stone-500">
                {state.exactConnection
                  ? `Connected to division ${state.exactConnection.divisionCode}`
                  : "Not connected"}
              </p>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <ActionButton
                  ...
                  disabledReason={
                    !hasPermission("connect_exact")
                      ? "Your role cannot connect Exact Online."
                      : "Exact Online connection is in progress."
                  }
                >
                  {state.exactConnection ? "Reconnect" : "Connect Exact Online"}
                </ActionButton>
                <ActionButton
                  variant="secondary"
                  ...
                  disabledReason={
                    !hasPermission("connect_exact")
                      ? "Your role cannot sync Exact Online data."
                      : "Connect Exact Online before syncing Exact data."
                  }
                >
                  Sync Exact Data Now
                </ActionButton>
                <ActionButton
                  ...
                  feedback={buttonFeedbackFor("exact-disconnect", "exact-disconnect")}
                  disabled={
                    !state.exactConnection ||
                    busy === "exact-disconnect" ||
                    !hasPermission("connect_exact")
                  }
                  disabledReason={
                    !state.exactConnection
                      ? "Exact Online is not connected."
                      : "Your role cannot disconnect Exact Online."
                  }
                >
                  Disconnect Exact
                </ActionButton>
            ...
            <div className="rounded-lg border border-stone-300 bg-white p-4">
              <h2 className="text-lg font-semibold">Outlook ingestion</h2>
              <p className="mt-1 text-sm text-stone-500">
                {state.outlookConnection
                  ? `Connected to ${state.outlookConnection.mailboxAddress}`
                  : "Not connected"}
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                <ActionButton
                  ...
                  disabled={
                    busy === "outlook-connect" || !hasPermission("connect_outlook")
                  }
                  disabledReason={
                    !hasPermission("connect_outlook")
                      ? "Your role cannot connect Outlook."
                      : "Outlook connection is in progress."
                  }
                >
                  {state.outlookConnection ? "Reconnect" : "Connect Outlook"}
                </ActionButton>
                <ActionButton
                  variant="secondary"
                  ...
                  disabledReason={
                    !hasPermission("upload") || !hasPermission("connect_outlook")
                      ? "Your role cannot scan Outlook invoices."
                      : "Scan Outlook Now is disabled until Outlook is connected."
                  }
                >
                  Scan Outlook Now
                </ActionButton>
                <ActionButton
                  ...
                  disabled={
                    !state.outlookConnection ||
                    busy === "outlook-disconnect" ||
                    !hasPermission("connect_outlook")
                  }
                  disabledReason={
                    !state.outlookConnection
                      ? "Outlook is not connected."
                      : "Your role cannot disconnect Outlook."
                  }
                >
                  Disconnect Outlook
                </ActionButton>
        ...
        <section className="rounded-lg border border-stone-300 bg-white">
          <div className="flex flex-col gap-3 border-b border-stone-200 p-4 md:flex-row md:items-center md:justify-between">
            ...
            <ActionButton
              ...
              disabledReason={
                !hasPermission("book")
                  ? "Your role does not allow booking invoices."
                  : !state.exactConnection
                  ? "Connect Exact Online before booking ready invoices."
              ...
              <div className="flex flex-col gap-4 p-4">
                  ...
                  <section className="space-y-4 border-y border-stone-200 py-4">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                      ...
                      <ActionButton
                        ...
                        disabledReason={
                          !hasPermission("approve")
                            ? "Your role does not allow approving purchase journal intelligence."
                ...
                <div className="flex flex-wrap gap-2">
                  <ActionButton
                    ...
                    disabledReason={
                      !hasPermission("edit")
                        ? "Your role does not allow editing invoices."
